...
class SPBprojectCollection extends ResourceCollection
{
                ...
                'produk' => $spbProject->productCompanySpbprojects->map(function ($product) use ($spbProject) {
                            ...
                            if ($status === ProductCompanySpbProject::TEXT_PAID_PRODUCT) {
                                ...
                                return [
                                    ...
                                    'vendor' => [
                                       'id' => $product->company->id ?? null,
                                        'name' => $product->company->name ?? null,
                                        'bank_name' => $product->company->bank_name ?? null,
                                        'account_name' => $product->company->account_name ?? null,
                                    ],
    ...
    protected function getUpdatedAt($spbProject)
    {
        ...
        if (in_array($category->id, [SpbProjectCategory::INVOICE, SpbProjectCategory::FLASH_CASH])) {
            ...
            // 🔹 Jika ada vendor yang sudah PAID, tetapi belum TAB_PAID, tampilkan "Pembayaran Sudah Sebagian"
            return "Pembayaran Sudah Sebagian";
        }
